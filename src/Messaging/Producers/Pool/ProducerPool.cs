...
namespace KsqlDsl.Messaging.Producers.Pool
{
    public class ProducerPool : IDisposable
    {
        ...
        private readonly IKsqlConfigurationManager _configManager;  // ✅ 追加
            ...
            IKsqlConfigurationManager configManager)
        {
            ...
            _configManager = configManager;  // ✅ 初期化追加
        ...
        {
            var busOptions = _configManager.GetOptions<KafkaBusOptions>();
            var producerOptions = _configManager.GetOptions<KafkaProducerOptions>();

            var config = new ProducerConfig
            {
                BootstrapServers = busOptions.BootstrapServers,
                Acks = (Confluent.Kafka.Acks)producerOptions.Acks,
                EnableIdempotence = producerOptions.EnableIdempotence,
                MaxInFlight = producerOptions.MaxInFlightRequestsPerConnection,
                CompressionType = (Confluent.Kafka.CompressionType)producerOptions.CompressionType,
            ...
            {
                config.Set(kvp.Key, kvp.Value?.ToString() ?? string.Empty);
            }
